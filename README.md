...
Extracted entities and relevant context are passed to Google Gemini via the Message a Model node.

The model generates a polished, professional reply acknowledging the inquiry and providing the application portal link.

The reply is sent back using the Gmail Send node.
...
Upload the included workflow.json file
